...
class YesIntentHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        try:
            if is_game_over(handler_input.attributes_manager.session_attributes["stats_record"],handler_input.attributes_manager.session_attributes["country"],handler_input) == False:
                reprompt_output = YES_OR_N0_REPROMPTS[randint(0, len(YES_OR_N0_REPROMPTS)-1)]
...
class SpeakToGuideIntentHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        if is_user_on_session(handler_input):
            ...
            speak_output = "<voice name=\""+ get_polly_voice(handler_input.attributes_manager.session_attributes["country"]) + "\">Hello adventurer! " + tip_for_question + "</voice> " + next_question  
            reprompt_output = "<voice name=\""+ get_polly_voice(handler_input.attributes_manager.session_attributes["country"]) + "\"> Adventurer, don't hesitate! " + tip_for_question + "</voice> " + next_question 
            include_display(handler_input)
        else:
        ...
        if handler_input.request_envelope.request.status.code == "200":
            if is_returning_user(handler_input) and has_active_adventure(handler_input):
                if handler_input.request_envelope.request.payload.get("purchaseResult") == PurchaseResult.DECLINED.value:
                    ...
                    speech = ("Your travel tip is: {}. {}".format(get_tip_for_question(handler_input.attributes_manager.session_attributes["country"], handler_input.attributes_manager.session_attributes["stats_record"], handler_input),
                            get_next_question(handler_input.attributes_manager.session_attributes["country"], handler_input.attributes_manager.session_attributes["stats_record"],handler_input)))
                    reprompt = YES_OR_N0_REPROMPTS[randint(0, len(YES_OR_N0_REPROMPTS)-1)]  
                    return response_builder.speak(speech).ask(reprompt).response
        else:
...
class FallbackIntentHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        speech = (
                ...
                "continue on your adventure by saying Italy or Australia. "
            )
...
def getYesorNoResponse(handler_input, textType):
    ...
    try: 
        ...
        if question_record['Count'] == 1: 
            ...
            if is_game_over(handler_input.attributes_manager.session_attributes["stats_record"],handler_input.attributes_manager.session_attributes["country"],handler_input):
...
def get_next_question(countryname, stats, handler_input):
    ...
    question_record = table.query(KeyConditionExpression=Key('CountryId').eq(get_country_id(countryname)) & Key('QuestionNumber').eq(stats['Items'][0]['QuestionNumber']+1)) #current completed question + 1
...
def is_game_over(stats, country, handler_input):
    #game is over if they run out of wealth or energy or there are no questions left
    if stats['Items'][0]['MoneyLevel'] <= 0 or stats['Items'][0]['EnergyLevel'] <=0 or stats['Items'][0]['ActiveFlag'] == 'N' or get_next_question(country,stats,handler_input)  == GAME_END:
        return True
    else:
        return False
